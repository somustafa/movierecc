...
st.markdown("""
    ...
    h1 { 
        ...
        font-size: 90px !important; /* Daha böyük head */
    ...
    label[data-testid="stWidgetLabel"] p {
        font-size: 25px !important; /* SEARCH FILMS yazısı */
        font-family: 'Jockey One', sans-serif !important;
        color: #E9C46A !important;
    }
    ...
    /* Düyməni posterin üzərinə tam yaymaq */
    ...
    /* Seçiləndə posterin rənginin dəyişməsi */
    .selected-overlay {
        position: absolute;
        top: 0; left: 0; width: 100%; height: 100%;
        background-color: rgba(196, 30, 58, 0.4); /* Qırmızımtıl filtr */
    ...
    /* Balaca və Sadə Precision Yazısı */
    .precision-text {
        font-family: 'Jockey One', sans-serif;
        color: #E9C46A;
        text-align: center;
...
if st.session_state.page == "home":
    ...
    for i, (idx, row) in enumerate(st.session_state.main_pool.iterrows()):
        ...
        with cols[i % 5]:
            ...
            if is_sel:
                st.markdown('<div class="selected-overlay"></div>', unsafe_allow_html=True)
                st.markdown('<div class="check-mark">SELECTED</div>', unsafe_allow_html=True)
            
            st.image(get_poster_cached(row['clean_title']), use_container_width=True)
            
            # Tam poster klik sahəsi
            st.markdown('<div class="poster-click-area">', unsafe_allow_html=True)
            if st.button("", key=f"p_{m_id}"):
                if m_id in st.session_state.selected_ids:
                    st.session_state.selected_ids.remove(m_id)
                else:
                    st.session_state.selected_ids.add(m_id)
                st.rerun()
...
# --- RESULTS ---
    ...
    # Precision Score (Sadə yazi kimi)
    ...
    st.markdown(f"<div class='precision-text'>PRECISION SCORE: {(hits/10)*100}%</div>", unsafe_allow_html=True)